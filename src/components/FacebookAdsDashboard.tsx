...
const FacebookAdsDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setError('Failed to fetch Facebook Ads data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const formatCurrencyWithConversion = (amount: string | number, currency: string) => {
  ...
  const processCampaignsWithInsights = () => {
    ...
    return data.campaigns.map((campaign: any) => {
      ...
      const campaignAds = data.ads.filter((ad: any) => ad.campaign_id === campaign.id)
      ...
      const aggregatedInsights = campaignAds.reduce((acc: any, ad: any) => {
        if (ad.insights && ad.insights.data && ad.insights.data.length > 0) {
          ...
          return {
            impressions: (acc.impressions || 0) + (Number(insight.impressions) || 0),
            ...
            frequency: Math.max(acc.frequency || 0, Number(insight.frequency) || 0), // Take max frequency
            cpc: ((acc.cpc || 0) * (acc.clickCount || 0) + (Number(insight.cpc) || 0) * (Number(insight.clicks) || 0)) / ((acc.clickCount || 0) + (Number(insight.clicks) || 0)) || 0, // Weighted average
            cpm: ((acc.cpm || 0) * (acc.impressionCount || 0) + (Number(insight.cpm) || 0) * (Number(insight.impressions) || 0)) / ((acc.impressionCount || 0) + (Number(insight.impressions) || 0)) || 0, // Weighted average
            ctr: ((acc.ctr || 0) * (acc.impressionCount || 0) + (Number(insight.ctr) || 0) * (Number(insight.impressions) || 0)) / ((acc.impressionCount || 0) + (Number(insight.impressions) || 0)) || 0, // Weighted average
            clickCount: (acc.clickCount || 0) + (Number(insight.clicks) || 0),
            impressionCount: (acc.impressionCount || 0) + (Number(insight.impressions) || 0)
          }
        }
        return acc
      }, {})
  ...
  return (
    <div className="facebook-ads-dashboard">
      ...
      <div className="dashboard-grid">
        ...
        />
        {data?.account && (
          <AccountOverviewWidget account={data.account} />
        )}
        <PerformanceMetricsWidget
          totals={totals}
          currency="USD"
